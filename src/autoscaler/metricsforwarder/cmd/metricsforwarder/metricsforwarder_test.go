		...
		Context("when a request with custom metrics comes", func() {
			Context("when using polling for metrics collection", func() {
				BeforeEach(func() {
					...
					body, err := json.Marshal(models.CustomMetric{Name: "custom", Value: 12, Unit: "unit", InstanceIndex: 1, AppGUID: "an-app-id"})
					Expect(err).NotTo(HaveOccurred())
					
					req, err = http.NewRequest(http.MethodPost, fmt.Sprintf("http://127.0.0.1:%d/v1/metrics",cfg.ServerPort), bytes.NewReader(body))
					Expect(err).NotTo(HaveOccurred())
